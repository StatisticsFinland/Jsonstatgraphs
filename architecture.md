...
6. **Transform** — Each transformer projects the rebuilt N-dimensional dataset into its renderer-specific shape. `transform.ts` creates categorical series and X-axis combinations, including composite codes and labels when an axis contains multiple dimensions. `table-transform.ts` creates an N-dimensional pivot grid. Scatter, pyramid, map, and key-figure transforms select their required coordinates directly from the rebuilt cube. Unconfigured datasets retain their existing automatic transformation behavior.
...
10. **Render** — The chart-specific renderer draws the projected data into the plot zone. Scatter metric/content dimensions and pyramid split dimensions cannot be selectable because they define required renderer structure.
...
The rebuilt dataset removes consumed `extension.selectableConfig` data so selections are not applied twice, while preserving roles, dimension metadata, and unrelated extension metadata. Unconfigured datasets never allocate a rebuilt cube and retain the original transformation path. Explicit chart choices are honored after structural dataset validation; automatic selection remains responsible for choosing a compatible chart type.
