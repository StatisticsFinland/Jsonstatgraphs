...
6. **Transform** — Each transformer projects the rebuilt N-dimensional dataset into its renderer-specific shape without rebuilding it again. `transform.ts` creates categorical series and X-axis combinations, including composite codes and labels when an axis contains multiple dimensions. `table-transform.ts` creates an N-dimensional pivot grid and hides all singleton dimensions, including those named in `layout`; an all-singleton dataset remains a scalar grid. Scatter, pyramid, map, and key-figure transforms select their required coordinates directly from the rebuilt cube. Unconfigured datasets retain their existing automatic transformation behavior. `sorting.ts` applies `ChartConfig.sorting` only to horizontal bar, grouped/stacked/percent horizontal bar, and pie charts. Keywords reorder outer categories; matching series codes are used by grouped/stacked/percent horizontal bars, with grouped horizontal bars prioritizing the matching series.
