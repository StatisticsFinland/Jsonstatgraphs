...
12. **Accessibility** — Visual charts expose one localized, named `region`. Interactive datapoints use localized list semantics and a chart-orientation-aware roving tab stop that is preserved across redraws. Arrow events are owned only by focused datapoints and stop at boundaries; Tab remains the focus-exit mechanism. Decorative SVG content is hidden from assistive technology. The library does not append hidden data tables: host applications own alternate data presentations, while the explicit chart/table menu mode remains available.
