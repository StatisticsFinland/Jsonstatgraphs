...
7. **Build headers** — `index.ts` resolves active selectable categories, then `header-builder.ts` constructs the automatic title and selection subtitle from metadata. Single selected categories appear in automatic titles and in subtitles for selectable dimensions; dimensions with zero or multiple selected categories are omitted from subtitles. Key figures intentionally skip automatic subtitle generation because their renderer has no subtitle presentation. A configured subtitle overrides the generated value for renderers that support subtitles, while a configured multi-select dimension remains a variable in the title when multiple values are active.
...
12. **Accessibility** — Visual charts expose one localized, named `region` containing a localized, named SVG `application` so screen readers enter focus mode for the custom keyboard surface. Interactive datapoints use localized list semantics and a dataset-series roving tab stop: the initial entry is the first point of the first series, Right/Down move forward through points, Left/Up move backward, and forward/backward navigation advances across series boundaries. Home/End select the first/last point of the current series; focus is preserved on re-entry and redraw. Arrow events are owned only by focused datapoints and retain focus at unavailable boundaries; Tab remains the focus-exit mechanism. Table and key-figure views retain native document semantics. Decorative SVG content is hidden from assistive technology. The library does not append hidden data tables: host applications own alternate data presentations, while the explicit chart/table menu mode remains available.
