...
│   ├── sorting.ts        Category sort order for bar/pie charts (sum, ascending/descending, reference series, reversed)
...
6. **Transform** — Each transformer projects the rebuilt N-dimensional dataset into its renderer-specific shape without rebuilding it again. `transform.ts` creates categorical series and X-axis combinations, including composite codes and labels when an axis contains multiple dimensions. `table-transform.ts` creates an N-dimensional pivot grid. Scatter, pyramid, map, and key-figure transforms select their required coordinates directly from the rebuilt cube. Unconfigured datasets retain their existing automatic transformation behavior. `sorting.ts` then reorders the transformed categorical series for bar and pie chart types per `ChartConfig.sorting`; other chart types ignore it.
