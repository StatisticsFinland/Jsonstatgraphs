...
### ADR-040: Application boundary for interactive visual charts

**Status:** accepted (amends ADR-038 and ADR-039)
**Date:** 2026-09-07

**Context:** The roving-tabindex navigation introduced by ADR-038 works only when a screen reader passes arrow-key events to the page. NVDA can leave users in browse mode when focus enters an SVG `listitem`, causing its virtual cursor to consume the same arrow keys that navigate chart datapoints. A named `region`, focusable SVG elements, and keyboard instructions describe the chart but do not reliably signal that the surface requires focus mode.

**Decision:** Interactive visual chart and map SVG roots use a localized, named `role="application"` inside the existing localized `role="region"` container. Datapoints retain their list/listitem semantics and roving tab stop. Keyboard events remain owned only by registered datapoints, and Tab/Shift+Tab are never intercepted. The application does not include keyboard instructions because repeating them for every chart creates excessive verbosity on pages with multiple charts. Table and key-figure renderers retain native document semantics and do not use an application role.

**Consequences:** Screen readers such as NVDA receive an explicit signal to enter focus mode when users Tab into chart data, allowing the library's arrow navigation to operate. Application semantics intentionally cover only visual chart SVGs, not the surrounding host application or native table content. Automated tests verify the semantic hierarchy, localization, event ownership, and Tab exit, but releases must also verify actual mode switching with NVDA in Chromium and Firefox. The library still does not generate hidden tables; consumers should expose the chart menu's table mode or provide a visible host-owned table when an alternate data presentation is required.
